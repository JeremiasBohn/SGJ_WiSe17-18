...
*/
	...
	IEnumerator Phase1() {
		...
		while (phase==1) {
			...
			case 6:
				...
				break;
			case 7: 
				yield return StartCoroutine (BoobiePunch ());
				break;
			default:
	...
	IEnumerator Phase0() {
		yield return new WaitForSeconds (2f);
		Vector2 spawnPoint;
		Debug.Log (bitchSquad.Length + " huttrenohn");
		...
		if (phase != 0) {
			//	for (int i = 0; i < bitchSquad.Length; i++) {
				//Destroy (bitchSquad [i].gameObject);
				//Destroy (GameObject.Find ("Enemy_porting(Clone)"));
		//		}
			//	Destroy (GameObject.Find ("Enemy_porting(Clone)"));
		//	for (int i = 0; i < bitchSquad.Length; i++) {
		//		bitchSquad [i].SetActive (false);
		//	}
			GameObject[] all = GameObject.FindGameObjectsWithTag("plox die");
